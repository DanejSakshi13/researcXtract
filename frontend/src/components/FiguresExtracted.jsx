...
const FiguresExtracted = ({ figures }) => {
  ...
  const handlePrev = () => {
    setCurrentIndex((prevIndex) => (prevIndex > 0 ? prevIndex - 1 : totalFigures - 1));
  };

  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex < totalFigures - 1 ? prevIndex + 1 : 0));
  };
  ...
  const imageSrc = currentFigure.image ? `data:image/${currentFigure.type};base64,${currentFigure.image}` : '';

  return (
    <Card>
      <Title>Figures</Title>
      <CarouselContainer>
        <ArrowButton onClick={handlePrev} disabled={totalFigures <= 1}>
          <ArrowBackIcon />
        </ArrowButton>
        <ContentWrapper>
          {imageSrc ? (
            <FigureImage src={imageSrc} alt={currentFigure.caption || `Figure ${currentIndex + 1}`} />
          ) : (
            <p>Image not available.</p>
          )}
          <Caption>{currentFigure.caption || `Figure ${currentIndex + 1}`}</Caption>
        </ContentWrapper>
        <ArrowButton onClick={handleNext} disabled={totalFigures <= 1}>
          <ArrowForwardIcon />
        </ArrowButton>
      </CarouselContainer>
    </Card>
